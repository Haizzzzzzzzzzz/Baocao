$ten=$_POST['name'];
        ...
        if (isset($ten)&&isset($email)&&isset($sdt)&&isset($diachi)) { 
            if(isset($_SESSION['giohang'])){
                $arrid=array();
                $i_custom = 0;
                $reversed_custom = array_reverse($arr_custom);
                ...
                foreach ($_SESSION['giohang'] as $id_sp => $sl) {
                    ...
                    $sql1="SELECT gia_sp as giaSP FROM SanPham WHERE id_sp='$id_sp'";
                    $query_gia_sp = mysqli_query($conn,$sql1);
                    $row_gia_sp = mysqli_fetch_array($query_gia_sp);
                    $gia_tien = $row_gia_sp['giaSP'];
                    
                    $sql="INSERT INTO donhang(id_dh, ten_sp, custom_text, ten, dien_thoai,dia_chi,email,id_sp, ngay_gio, gia_tien) VALUES('$i','$ten_sp', '$customText' ,'$ten','$sdt',"."'$diachi','$email','$id_sp','$ngay_gio','$gia_tien')";
       ...
       $strBody='';
        ...
        $mailer->AddCC("caiginhi103@gmail.com", "Admin JERSEY CUSTOM"); // gửi thêm một email cho Admin

        // Chuẩn bị gửi thư nào
        $mailer->setFromName = 'Shop JERSEY CUSTOM'; // tên người gửi
        $mailer->setFrom = 'caiginhi103@gmail.com'; // mail người gửi
        $mailer->Subject = 'Hóa đơn xác nhận mua hàng từ JERSEY CUSTOM';
